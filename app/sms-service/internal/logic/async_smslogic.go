...
func (s *ASyncSmsLogic) SendSms(ctx context.Context, phone string, args map[string]string) (string, error) {
	var (
		brand string
		err   error
	)
	...
	start := time.Now()
	logx.Infof("[ASyncSmsLogic] 当前短信发送延迟小于200ms, 同步发送短信[id=%d]", record.ID)
	if brand, err = s.smsService.SendSms(ctx, phone, args); err != nil {
		logx.Errorf("[ASyncSmsLogic] 同步发送短信[id=%d]异常改为异步投送：%s", record.ID, err)
		return brand, s.kafkaProducer.SendSync(ctx, topic, strconv.Itoa(record.ID))
	}
...
func (s *ASyncSmsLogic) calculateMedianLatency() int64 {
	...
	if n == 0 {
		...
		// Bug fix: Ensure that the index does not exceed the slice length
